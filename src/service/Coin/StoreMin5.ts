...
export class BittrexMin5Trading {
  _id?: Uuid
  name: string
  market: string
  key: string
...
export default class StoreMin {
  ...
  static readonly MIN_TIME = [2, 5, 10]

  private static lastUpdateDB

  static async find(fil) {
    return await StoreMin.mongo.find<BittrexMin5Trading>(BittrexMin5Trading, fil)
  }
  ...
  static async insert(tradings: BittrexCachedTrading[], now: Date) {
    ...
    if (!StoreMin.lastUpdateDB || (StoreMin.lastUpdateDB.getMinutes() !== now.getMinutes() && now.getMinutes() % 5 === 0)) {
      ...
      for (let e of tradings) {
        ...
        const tr = {} as BittrexMin5Trading
        tr._id = e._id
        tr.key = e.key
        tr.name = e.name
        tr.market = e.market
