...
export class GlobalService {
  static async export({ projectId }) {
    try {
      await Utils.executeCmd(`mongodump -d files -c Files -q "{'project_id': ObjectId('${projectId}') }" -o "${Utils.getTempPath(projectId.toString())}"`)
      await Utils.executeCmd(`mongodump -d files -c FileConfig -q "{'project_id': ObjectId('${projectId}') }" -o "${Utils.getTempPath(projectId.toString())}"`)
