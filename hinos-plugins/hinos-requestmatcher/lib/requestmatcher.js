...
function MATCHER({ query = null, params = null, body = null }) {
    return function (target, fieldName, func) {
        ...
        func.value['mid'].splice(0, 0, async (ctx, next) => {
            let handle = [];
            if (params)
                ctx.params = await applyValue(params, ctx.params);
            if (query)
                ctx.query = await applyValue(query, ctx.query);
            if (body) {
                if (!(body instanceof Array) && !(ctx.body instanceof Array)) {
                    ...
                    const err = new Error('Matcher is not valid');
                    err.status = 406;
                    throw err;
                }
            }
...
async function applyValue(schema, body) {
    let type, value, rs = {};
    if (body) {
