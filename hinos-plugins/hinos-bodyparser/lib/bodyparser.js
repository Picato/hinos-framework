...
function bodyParser(ctx, type) {
    ...
    return new Promise((resolve, reject) => {
        ...
        req.on('end', function () {
            ...
            if (type === 0)
                return resolve(JSON.parse(queryData.join('')));
            const str = queryData.join('');
            if (!/\[\d+\]/.test(str))
                return resolve(querystring.parse(str));
            resolve(convertToFormArray(querystring.parse(str)));
        });
    });
}
function bodyRawParser(ctx) {
    const req = ctx.req;
    return new Promise((resolve, reject) => {
...
async function bodyPartParser(ctx, options) {
    ...
    const storage = multer.diskStorage({
        destination: function (req, file, cb) {
            ...
            if (!opt)
                return cb(`Could not found config file for ${file.fieldname}`);
            file.opt = opt;
            cb(null, opt.uploadDir);
        },
        filename: function (req, file, cb) {
            const opt = file.opt;
            const fileName = uuid_1.v4() + path.extname(file.originalname);
    ...
    for (let opt of options) {
        if (opt.maxCount > 1) {
            ...
            if (!opt.returnType || opt.returnType === String)
                body[opt.name] = body[opt.name].map(e => e.httpPath);
        }
        else {
            body[opt.name] = req['files'][opt.name][0];
            if (opt.resize)
                await resizeImage(body[opt.name].path, opt.resize);
            if (!opt.returnType || opt.returnType === String)
                body[opt.name] = body[opt.name].httpPath;
        }
    }
    req['files'] = null;
    req['body'] = null;
...
async function resizeImage(file, sizes) {
    ...
    const resizeImg = (size) => {
        return new Promise((resolve, reject) => {
            try {
                if (!size.w && !size.h)
                    return reject('Need enter size to resize image');
                ...
                if (!size.w)
                    size.w = size.h * image.bitmap.width / image.bitmap.height;
                if (!size.h)
                    size.h = size.w * image.bitmap.height / image.bitmap.width;
                if (size.ratio) {
                    size.w *= size.ratio;
                    size.h *= size.ratio;
                }
